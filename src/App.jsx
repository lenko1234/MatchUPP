...
const App = () => {
    ...
    const OwnerScheduleView = () => {
        ...
        const occupiedSlots = filteredSlots.filter(s => s.status !== 'available').length;
        const totalSlots = filteredSlots.length;

        return (
            <div className="bg-slate-50 min-h-screen pb-24 animate-in fade-in duration-500">
                {/* Header Fijo */}
                <header className="bg-white p-6 pb-2 rounded-b-[2.5rem] shadow-sm">
                    <div className="flex justify-between items-center mb-6">
                        <h1 className="text-2xl font-black text-slate-800 flex items-center gap-2">
                            <Calendar size={28} className="text-emerald-600" />
                            Febrero <span className="font-light text-slate-400">2026</span>
                        </h1>
                        <button className="bg-slate-100 p-2.5 rounded-2xl text-slate-500 hover:bg-emerald-50 hover:text-emerald-600 transition-colors">
                            <Settings size={20} />
                        </button>
                    </div>
                ...
                <div className="p-6 space-y-6">
                    <div className="flex justify-between items-center">
                        <h2 className="font-bold text-slate-400 text-xs uppercase tracking-tighter">Agenda de hoy</h2>
                    ...
                    <div className="space-y-4 relative before:absolute before:left-[19px] before:top-4 before:bottom-0 before:w-0.5 before:bg-slate-200">
                        {filteredSlots.map((slot, idx) => (
                            <div key={idx} className="flex gap-6 items-start group relative">
                                {/* Hora */}
                                <div className="text-[11px] font-black text-slate-400 w-10 pt-4 bg-slate-50 z-10">
                                    {slot.time}
                                </div>

                                {/* Card de Turno */}
                                <div className={`flex-1 rounded-[1.8rem] p-4 border-2 transition-all duration-300 ${slot.status === 'available'
                                    ? 'bg-emerald-50/30 border-dashed border-emerald-200 hover:border-emerald-500 cursor-pointer'
                                    ...
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <h3 className={`font-bold text-sm ${slot.status === 'available' ? 'text-emerald-700' : 'text-slate-800'}`}>
                                        ...
                                        {slot.status === 'available' && (
                                            <div className="text-right">
                                                <span className="text-xs font-black text-emerald-600">$1500</span>
                                                <p className="text-[8px] font-bold text-emerald-400 uppercase">Seña $500</p>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    {slot.status === 'matchmaking' && (
                                        <div className="mt-4">
                                            <div className="flex justify-between text-[10px] mb-1 font-bold">
                                                <span className="text-emerald-600">{slot.players}/{slot.total} jugadores</span>
                                                <span className="text-slate-400">Faltan {slot.total - slot.players}</span>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-2">
                                                {slot.paid ? (
                                                    <>
                                                        ...
                                                        <span className="text-xs font-bold text-emerald-600">Pagado</span>
                                                    </>
                                                ) : (
                                                    <>
                ...
                <button
                    onClick={() => setShowCreateModal(true)}
                    className="fixed bottom-8 right-8 w-14 h-14 bg-slate-900 text-white rounded-full shadow-2xl flex items-center justify-center hover:scale-110 active:scale-90 transition-all border-4 border-white z-50"
                >
                    <PlusCircle size={24} />
                </button>
            </div>
